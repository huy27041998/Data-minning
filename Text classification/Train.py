...
class Train:
    ...
    def train_1_class(self):
        ...
        for train_size in train_sizes:
            print('Training size:', math.floor(train_size * 0.75) * 2, 'Test size:', math.ceil(train_size * 0.25) * 2)
            ...
            Y_train = [1 if i == 'business' else 0 for i in self.train_data['class_name']]
            X_test = vect.transform(self.test_data['data'])
            Y_test = [1 if i == 'business' else 0 for i in self.test_data['class_name']]
            nb = MultinomialNB()
    ...
    def train_n_classes(self):
        ...
        for train_size in train_sizes:
            print('Training size:', math.floor(train_size * 0.75) * 5, 'Test size:', math.ceil(train_size * 0.25) * 5)
            self.loadData(train_size)
            vect = TfidfVectorizer(stop_words=stopWords)
            self.train_and_test_split(0.75)
            classes = {}
